	...
	<body>
		...
		<main role="main" class="container">
			<div class="container" style="margin-top: 6rem;">
                <form method="post" action="${contextPath}/people" id="person-criteria">
                    <div class="row">
                        <div class="col-md-11 col-8">
                            <div class="input-group mb-5">
                                <input type="text" class="form-control" name="name" placeholder="Wyszukaj osobę..."
                    ...
                    <div class="collapse rounded box-colors shadow mb-5 px-5 py-2" id="collapseSearch">
                        ...
                        <div class="form-group row my-3">
                            ...
                            <div class="col-sm-10 form-group">
                                <select class="selectpicker" name="professions" title="Wybierz zawód..." data-width="100%" data-size="8">
                                    <c:forEach items="${professions}" var="profession">
                                        <option value="${profession.key.id}" <c:if test="${profession.value.booleanValue()}">selected</c:if>>
                                            ${profession.key.name}
                                        </option>
                                    </c:forEach>
                                </select>
                        ...
                        <div class="form-group row my-3">
                            ...
                            <div class="col-sm-10 form-group">
                                <select class="selectpicker" name="sort_by" title="" data-width="100%">
                                    ...
                                    <optgroup label="rok urodzenia">
                                        <option value="date-descending" title="rok urodzenia: najpóźniej"
                                            <c:if test="${not empty sort_by && sort_by == 'date-descending'}">selected</c:if>>
                                            najpóźniej
                                        </option>
                                        <option value="date-ascending" title="rok urodzenia: najwcześniej"
				...
				<div class="row card-columns justify-content-center">
                    <c:if test="${empty people}"><h1 class="green-font-color">Nie znaleziono żadnych osób</h1></c:if>

					<c:forEach items="${people}" var="person">
						<div class="col-xl-4 col-md-6 col-sm-12">
							<div class="card mb-5 film-card">
								<img class="card-img-top" src="${posterPaths[person.id]}" alt="Card image cap">
								<div class="card-body">
									<h4 class="card-title"><a href="${contextPath}/people/${person.id}" class="green-font-color film-link">${person.stageName}</a></h4>
								...
								<div class="card-footer text-right box-colors">
									<span class="orange-font-color">★</span> <span class="text-muted">
									    ${empty person.averageRating ? "Brak" : person.averageRating}
									</span>
								</div>
							</div>
						</div>
                ...
                <div class="pagination justify-content-center mb-5">
                    <c:forEach items="${pagination}" var="p" varStatus="loop">
                        <c:if test="${loop.first && p != page}">
                            <button name="page" value="${page - 1}" form="person-criteria">&laquo;</button>
                        </c:if>

                        <button <c:choose>
                                    <c:when test="${p == page}">class="active" disabled</c:when>
                                    <c:otherwise>name="page" value="${p}"</c:otherwise>
                                </c:choose> form="person-criteria">
                            ${p}
                        </button>

                        <c:if test="${loop.last && p != page}">
                            <button name="page" value="${page + 1}" form="person-criteria">&raquo;</button>
                        </c:if>
                    </c:forEach>
                </div>
            </c:if>
